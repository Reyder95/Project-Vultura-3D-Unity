...
namespace Tayx.Graphy.Advanced
{
    public class G_AdvancedData : MonoBehaviour, IMovable, IModifiableState
    {
        ...
        public void SetPosition(GraphyManager.ModulePosition newModulePosition)
        {
            ...
            float xSideOffsetBackgroundImage;
            float ySideOffset;      

            if (m_rectTransform == null)
            {
                xSideOffsetBackgroundImage = 0;
                ySideOffset = 0;
            }
            else
            {
                xSideOffsetBackgroundImage    = Mathf.Abs(m_backgroundImages[0].rectTransform.anchoredPosition.x);
                ySideOffset                   = Mathf.Abs(m_rectTransform.anchoredPosition.y);
            }
